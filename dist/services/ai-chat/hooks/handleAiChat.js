...
const llm_resolver_1 = require("../../../adapters/ai/llm-resolver");
const conversation_store_1 = require("../conversation.store");
const team_orchestrator_1 = require("../agents/team-orchestrator");
const identity_agent_1 = require("../agents/identity-agent");
...
function buildMinimalContextFallback(walkthroughsData) {
    ...
    if (desc)
        return `Con el contexto publicado, el flujo más directo es "${flow.title}": ${desc}`;
...
function inferWalkthroughFromLastAssistantMessage(lastAssistantMessage, walkthroughList) {
...
function getRequesterContext(context) {
...
function buildSystemPrompt(assistantName, projectSystemPrompt) {
    let systemPrompt = '';
 ...
 * 2) Response agent
 */
const handleAiChat = async (context) => {
    ...
    const projectRows = await adapters_1.db
        .select({
    ...
    const projectWalkthroughs = await adapters_1.db
        .select({
        id: schema_1.walkthroughs.id,
        title: schema_1.walkthroughs.title,
        description: schema_1.walkthroughs.description,
        steps: schema_1.walkthroughs.steps,
        tags: schema_1.walkthroughs.tags,
        parentId: schema_1.walkthroughs.parentId,
        previousWalkthroughId: schema_1.walkthroughs.previousWalkthroughId,
        nextWalkthroughId: schema_1.walkthroughs.nextWalkthroughId,
    })
    ...
    const conversationHistory = conversation_store_1.conversationStore.formatHistory(projectId, {
        userId: requester.userId,
        actorSlug: requester.actorSlug,
        sessionId: requester.sessionId,
        assistantName,
    });
    const priorMessages = conversation_store_1.conversationStore.getHistory(projectId, {
        userId: requester.userId,
        actorSlug: requester.actorSlug,
        sessionId: requester.sessionId,
    });
    ...
    const conversationContext = conversationHistory
        ...
        : '';
    conversation_store_1.conversationStore.addMessage(projectId, 'user', message.trim(), {
        userId: requester.userId,
        actorSlug: requester.actorSlug,
        sessionId: requester.sessionId,
    });
    ...
    const llm = await (0, llm_resolver_1.resolveLLM)(orgId, projectId);
    ...
    const orchestrator = new team_orchestrator_1.AiChatTeamOrchestrator();
    const identityAgent = new identity_agent_1.IdentityAgent();
    const isAnonymous = requester.userId === 'anonymous';
    let profile = conversation_store_1.conversationStore.getProfile(projectId, {
        userId: requester.userId,
        actorSlug: requester.actorSlug,
        sessionId: requester.sessionId,
    });
    if (isAnonymous && !profile.userName && shouldAttemptIdentityCapture(message.trim(), lastAssistantMessage, profile.userTurns)) {
        const identity = await identityAgent.run({
            ...
            walkthroughs: projectWalkthroughs,
            contextSummary,
            llm,
            systemPrompt,
        });
        if (identity.hasProvidedName && identity.providedName) {
            conversation_store_1.conversationStore.setUserName(projectId, identity.providedName, {
                userId: requester.userId,
                actorSlug: requester.actorSlug,
                sessionId: requester.sessionId,
            });
            profile = conversation_store_1.conversationStore.getProfile(projectId, {
                userId: requester.userId,
                actorSlug: requester.actorSlug,
                sessionId: requester.sessionId,
            });
    ...
    const streamCallbacks = context.params?.streamCallbacks;
    let teamResult;
    try {
        ...
        teamResult = {
            ...
            responseType: 'answer',
            confidence: 0.2,
            message: buildMinimalContextFallback(projectWalkthroughs),
    ...
    if (isShortAffirmation(message.trim()) &&
        (!teamResult.suggestedWalkthroughIds || teamResult.suggestedWalkthroughIds.length === 0)) {
        const inferredIds = inferWalkthroughFromLastAssistantMessage(lastAssistantMessage, projectWalkthroughs.map((w) => ({ id: w.id, title: w.title })));
    ...
    const responseMessage = teamResult.message || buildMinimalContextFallback(projectWalkthroughs);
    conversation_store_1.conversationStore.addMessage(projectId, 'assistant', responseMessage, {
        userId: requester.userId,
        actorSlug: requester.actorSlug,
        sessionId: requester.sessionId,
    });
